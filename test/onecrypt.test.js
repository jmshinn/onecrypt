...
exports.keygen = {
	"default - symmetric": function (test) {
		test.expect(2);
	...
	"password - symmetric": function (test) {
		test.expect(2);
	...
	/*"asymmetric keygen": function (test) {
		test.expect(2);

		var rsa_key = onecrypt.gen_key({ asymmetric: true });
		public_pem = rsa_key.toPublicPem('binary');
		private_pem = rsa_key.toPrivatePem('binary');

		test.ok(public_pem.match(/^-----BEGIN PUBLIC KEY-----\n(.{64}\n){6}.+\n-----END PUBLIC KEY-----\n$/), 'RSA public key is the correct format');
		test.ok(private_pem.match(/^-----BEGIN RSA PRIVATE KEY-----\n(.{64}\n){24}.+\n-----END RSA PRIVATE KEY-----\n$/), 'RSA private key is the correct format');
...
/*exports.asymmetric = {
	"public to private": function (test) {
		test.expect();

		var pub = ursa.coercePublicKey(public_pem);
		var pvt = ursa.coercePrivateKey(private_pem);

		var cipherText = pub.encrypt(new Buffer(message), 'binary', 'base64', ursa.RSA_PKCS1_OAEP_PADDING);
		var plainText = pvt.decrypt(cipherText, 'base64', 'binary', ursa.RSA_PKCS1_OAEP_PADDING);
		test.equal(message, plainText);

		test.done();
	},
	"private to public": function (test) {
		test.expect();

		var pub = ursa.coercePublicKey(public_pem);
		var pvt = ursa.coercePrivateKey(private_pem);

		var cipherText = pvt.privateEncrypt(new Buffer(message), 'binary', 'base64');
		var plainText = pub.publicDecrypt(cipherText, 'base64', 'binary');
		test.equal(message, plainText);
...
exports.hashes = {
	"default message hash": function (test) {
		test.expect(1);
	...
	"buncha message hashes": function (test) {
	...
	"magento style password": function (test) {
		test.expect(2);
	...
	"sha1 password w/6 character salt": function (test) {
		test.expect(4);
	...
	"upgrade digest to encrypt password": function (test) {
		test.expect(3);
