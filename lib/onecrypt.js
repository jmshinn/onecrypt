...
// this code is non-functional at the moment, specifically the xor to set the interim key is not yet done correctly
/*function _pbkdf2(algo, payload, salt, iterations, keylen, callback) {
	...
	for (var block=1; block<=key_blocks; block++) {
		...
		for (var i=1; i<iterations; i++) {
			...
			hmac.end();
			hash = hmac.read();
			console.log(hash.toString('binary'));
			interim_key ^= hash;
		}

		console.log(interim_key);
