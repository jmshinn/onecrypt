...
var _gen_key = function(params) {
	...
	var keylen = params.asymmetric?2048:256; // 256 bits
	...
	if (params.asymmetric) {
		...
		// the returned object is not just a key but an object that provides most of the cryptographic functionality available in RSA
...
function hash(_opts, _hash) {
	...
	this.match = function(payload, secret, opts) {
		...
		if (opts.salt !== false && typeof opts.salt !== 'string' && ((typeof opts.delim === 'string' && opts.delim) || opts.salt_char_length)) {
...
exports.verify_stable = function(payload, secret, opts, callback) {
	...
	return verify(payload, secret, opts, callback);
};
