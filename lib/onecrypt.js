...
var _gen_key = function(params) {
	...
	var iterations = 128000; // default to 128000 iterations
...
var encrypt = exports.encrypt = function(payload, opts, callback) {
	...
	// 2^17 iterations, min recommended 2^14 (approx .1s), 2^20 for sensetive data (approx 5s), this should be tuned for
	// approximately .5s, which would be not too bad as a worst case user-experience, but a little more forward thinking
...
var encipher = exports.encipher = function(payload, key, mackey, opts, cb) {
	...
	if (opts.iv) {
		...
		var iv;
		if (typeof opts.iv === 'string') iv = opts.iv_encoding?new Buffer(opts.iv, opts.iv_encoding):new Buffer(opts.iv);
		else iv = opts.iv;

		setImmediate(function() { _encipher(payload, key, mackey, iv, opts, cb); });
	}
	else crypto.randomBytes(iv_length/8, function(err, iv) { _encipher(payload, key, mackey, iv, opts, cb); });
};
...
var _encipher = function(payload, key, mackey, iv, opts, cb) {
	...
	if (typeof payload === 'string') payload = opts.payload_encoding?new Buffer(payload, opts.payload_encoding):new Buffer(payload);
	if (typeof key === 'string') key = opts.key_encoding?new Buffer(key, opts.key_encoding):new Buffer(key);
	if (typeof mackey === 'string') mackey = opts.mackey_encoding?new Buffer(mackey, opts.mackey_encoding):new Buffer(mackey);

	var cipher = crypto.createCipheriv(opts.algorithm+'-'+opts.mode, key, iv);
	var ciphertext = cipher.update(payload);
...
var decipher = exports.decipher = function(payload, key, mackey, mac, iv, opts) {
	...
	if (typeof key === 'string') key = opts.key_encoding?new Buffer(key, opts.key_encoding):new Buffer(key);
	if (typeof mackey === 'string') mackey = opts.mackey_encoding?new Buffer(mackey, opts.mackey_encoding):new Buffer(mackey);
	if (typeof iv === 'string') iv = opts.iv_encoding?new Buffer(iv, opts.iv_encoding):new Buffer(iv);
	if (typeof mac === 'string') mac = opts.mac_encoding?new Buffer(mac, opts.mac_encoding):new Buffer(mac);
	...
	var plaintext = decipher.update(payload);
