		...
		if (callback) {
			if (opts.salt === true) {
				...
				crypto.randomBytes(saltlen, function(err, salt) {
					...
					digest.write(salt.toString(encoding)+payload);
					digest.end();
...
var match = exports.match = function(payload, secret, salt, opts) {
	if (typeof salt === 'object' && !Buffer.isBuffer(salt)) opts = salt, salt = undefined;
	else opts = opts || {};
	...
	if (!salt && ((typeof opts.delim === 'string' && opts.delim) || opts.salt_byte_length || opts.salt_bit_length || (opts.concat && opts.salt_byte_length = 16))) {
		...
		// 3) we've indicated that we've concatenated, and want to use the default bit length, 128
	}
	...
	if (opts.salt_after) payload += predelim + salt;
...
var digest_pair = exports.digest_pair = function(opts) {
};
...
var _generic_simple = function(opts, hash) {
	return function(payload, secret, salt) {
		...
		/* this is a weird convention, we should probably not handle boolean true, make a note to come back and look at this */
